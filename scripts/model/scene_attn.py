...
class PatchCrossAttnTransformerLayer(nn.Module):
    """Performs cross-attention between the occupancy grid and agents."""
    ...
    def __init__(
        ...
        self.num_patches=int((256/params.patch_size)**2)
    ...
    def forward(self, input_batch, scene_enc):
        ...
        scene_enc=scene_enc.unsqueeze(1)

        scene_enc = scene_enc.expand(b, a, self.num_patches, h)
        query = input_batch.reshape(b * a, t, h)
        key_value = scene_enc.reshape(b * a, self.num_patches, h)
