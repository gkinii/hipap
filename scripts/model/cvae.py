...
class TransformerCVAE(nn.Module):
    ...
    def sample(self, input_batch):

        # Get conditions from encoder (no future_batch during inference!)
        ...
        # Sample z from prior N(0, I) instead of using encoder output
        z = torch.randn(b, a, t, h, device=device)
...
class VAEEncoder(nn.Module):
    ...
    def forward(self, input_batch, future_batch=None):
        ...
        if future_batch is not None:
            ...
            combined = torch.cat([past_cond, future_enc], dim=-1)  # (b, a, 1, 2*h)

            # Fuse them
            enc_out = self.fusion_layer(combined)  # (b, a, 1, h)
